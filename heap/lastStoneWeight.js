 ...
 */
 var lastStoneWeight = function(stones) {
  ...
  while (maxHeap.heap.length >= 2) {
      let lastStone = maxHeap.dequeue();
      let secondLastStone = maxHeap.dequeue();
      if (lastStone > secondLastStone){
          maxHeap.enqueue(lastStone - secondLastStone);
      }
  }
  return maxHeap.heap.length ? maxHeap.dequeue() : 0;
};

class HeapItem {
constructor(val = 0, freq = val) {
  this.val = val;
  this.freq = freq;
}
}
